...
$(function(){
    ...
    if(userid!=null){
        $.getJSON("v1/info",{userid:userid},function(data){
            ...
            if(data.data == null){
                alert("invalid uer id，failed to initialize user main page");
                return;
            }
        ...
        layer.msg("Invalid parameter，please check website address"+userid);
    }
});
...
function changeUserText(user){
    ...
    $(".user-facebook").text(user.facebook);
...
function changeUserAddr(user){
    ...
    $(".user-twitter-href").attr("href",user.twitter);
    $(".user-twitter-href").attr("title",user.twitter);
    $(".user-facebook-href").attr("href",user.facebook);
    $(".user-facebook-href").attr("title",user.facebook);
    $(".user-linkedin-href").attr("href",user.linkedin);
    $(".user-linkedin-href").attr("title",user.linkedin);
    $("a.user-email").attr("href","mailto:"+user.email);
    $(".user-phone-href").attr("href","tel:"+user.phone);
    if(user.sex == null || user.sex == "male"){
...
function changeEdu(edus){
    for (let i=0;i<edus.length;i++){
        let $div = $("" +
            ...
            "                                            <div class=\"left-part\"><h5 class=\"item-period\">"+edus[i].start+" <br>to<br> "+edus[i].end+"</h5><span\n" +
...
function changeWork(works){
    for(let i=0;i<works.length;i++){
        let $div = $("" +
            "                                        <div class=\"timeline-item clearfix\">\n" +
            "                                            <div class=\"left-part\"><h5 class=\"item-period\">"+works[i].start+" <br>to<br> "+works[i].end+"</h5><span\n" +
